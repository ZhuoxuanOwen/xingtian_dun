...
public class StringUtil implements Serializable {
	...
	public static String convertChar2XMLEntity(String xmlSource) {
		xmlSource = StringUtil.clearString(xmlSource);
	 ...
	 * @return
	 */
	public static String format2SQLRLikeDialect(String fieldName,
		...
		for (int i = 0; i < conditions.length; i++) {
			sqlStringBuffer
					.append(fieldName + " like '" + conditions[i] + "%'");
	 ...
	 */
	public static String getCascadeName(String wbsCode, String separator,
			String name, String pad) {
	...
	public static String trimChar(String strTemp, String charTemp) {
		...
		if (!strTemp.equals("") && strTemp.length() > charTemp.length()
				&& strTemp.startsWith(charTemp)) {
			strTemp = strTemp
					.substring(charTemp.length(), strTemp.length() - 1);
		}
		if (!strTemp.equals("") && strTemp.length() > charTemp.length()
				&& strTemp.endsWith(charTemp)) {
			strTemp = strTemp
	 ...
	 */
	public static StringBuffer distinctAppend(StringBuffer buffer,
			String target, String seperator) {
		if (buffer.length() < 1) {
